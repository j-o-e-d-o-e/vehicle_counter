...
#height = frame.shape[0]
#length = frame.shape[1]
#INSERTED VALUES - joedoe
height = 360
length = 240

# Set the 6 dividers, formed by 1_A and 1_B
#INSERTED INT() - joedoe
DIVIDER1 = (DIVIDER1_A, DIVIDER1_B) = ((int(length / 3), height), (int(length / 3), int(290 * K)))
DIVIDER2 = (DIVIDER2_A, DIVIDER2_B) = ((int(length / 2), height), (int(length / 2), int(290 * K)))
DIVIDER3 = (DIVIDER3_A, DIVIDER3_B) = ((int(length / 3 * 2), height), (int(length / 3 * 2), int(290 * K)))
DIVIDER4 = (DIVIDER4_A, DIVIDER4_B) = ((int(length / 6), int(250 * K)), (int(length / 6), int(140 * K)))
DIVIDER5 = (DIVIDER5_A, DIVIDER5_B) = ((int(length / 3), int(250 * K)), (int(length / 3), int(140 * K)))
DIVIDER6 = (DIVIDER6_A, DIVIDER6_B) = ((int(length / 5 * 4), int(250 * K)), (int(length / 5 * 4), int(140 * K)))
...
def detect_vehicles(fg_mask):
    ...
    imagex, contours, hierarchy = cv2.findContours(fg_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    for entry in centroid_combined:
                ...
                #INSERTED INT() - joedoe
                centroid_aftercal.append((int(sum(tempx) / len(tempx)), int(sum(tempy) / len(tempy))))
